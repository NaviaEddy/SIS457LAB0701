...
void ANaveAereaJugador::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction(TEXT("Mina"), IE_Pressed, this, &ANaveAereaJugador::FireMina);
...
void ANaveAereaJugador::FireMina()
{
	bCanFire = true;
	UE_LOG(LogTemp, Warning, TEXT("Se presiono la M"));
	// Create fire direction vector
	...
	if (bCanFire == true)
	{
		...
		//{
			...
			if (World != nullptr)
			{
				...
				//World->SpawnActor<AMina>(SpawnLocation, FireRotation);
				//UE_LOG(LogTemp, Warning, TEXT("SpawnLocation(X, Y) = %s, %s FireRotation(X, Y) =  s, s"), SpawnLocation.X, SpawnLocation.Y);
				//UE_LOG(LogTemp, Warning, TEXT("World not nullptr"));
			}
...
void ANaveAereaJugador::FireShotMina(FVector FireDirection)
{
	// If it's ok to fire again
	if (bCanFire == true)
	{
